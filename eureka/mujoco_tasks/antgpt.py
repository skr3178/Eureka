...
class AntEnvGPT(MujocoEnv, utils.EzPickle):
    r"""
    MuJoCo Ant environment for Eureka.
    ...
    * v0: Initial versions release
    ...
    def step(self, action):
        """
        Override step to integrate LLM-generated reward function.
        """
        ...
        gt_reward, reward_info = self._get_rew(x_velocity, action)
        terminated = (not self.is_healthy) and self._terminate_when_unhealthy
        
        # Store ground-truth reward
        ...
        self.extras['gpt_reward'] = llm_reward
...
def compute_reward(
    ...
    '''
    Motivates the ant to run forward (positive x-direction) as fast as possible.
    The reward is based on the x-component of the torso's velocity (index 13 in observation).
    '''
    # Extract x-direction torso velocity (index 13 in observation vector)
    x_velocity = obs[..., 13]
    
    # Main reward component is forward velocity
    forward_vel_reward = x_velocity
    
    # Total reward consists of forward velocity component
    total_reward = forward_vel_reward
