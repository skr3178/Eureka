...
class SACAgent(BaseAlgorithm):
    ...
    def base_init(self, base_name, config):
        ...
        self.save_freq = config.get('save_frequency', 0)  # Initialize save_freq from save_frequency
    ...
    def train(self):
        ...
        while True:
            ...
            if self.game_rewards.current_size > 0:
                ...
                if self.save_freq > 0:
                    # Handle both scalar and array mean_rewards
                    mean_rewards_val = mean_rewards[0] if isinstance(mean_rewards, np.ndarray) and mean_rewards.ndim > 0 else mean_rewards
                    if (self.epoch_num % self.save_freq == 0) and (mean_rewards_val <= self.last_mean_rewards):
