...
def main(_) -> None:
    ...
    # Print coordinate system and axis information
    print("\n" + "=" * 70)
    print("COORDINATE SYSTEM & AXES INFORMATION")
    print("=" * 70)
    
    # Get hand positions and orientations
    if hasattr(env.task, 'left_hand') and hasattr(env.task, 'right_hand'):
        left_hand = env.task.left_hand
        right_hand = env.task.right_hand
        
        # Get hand root body positions
        physics = env.physics
        left_pos = physics.bind(left_hand.root_body).xpos
        right_pos = physics.bind(right_hand.root_body).xpos
        
        print(f"\nHand Positions (world coordinates):")
        print(f"  Left hand:  X={left_pos[0]:.3f}, Y={left_pos[1]:.3f}, Z={left_pos[2]:.3f}")
        print(f"  Right hand: X={right_pos[0]:.3f}, Y={right_pos[1]:.3f}, Z={right_pos[2]:.3f}")
        
        # Get forearm DOF information
        print(f"\nForearm DOFs (degrees of freedom):")
        for hand_name, hand in [("Left", left_hand), ("Right", right_hand)]:
            print(f"  {hand_name} hand:")
            forearm_tx = hand.mjcf_model.find("joint", "forearm_tx")
            forearm_ty = hand.mjcf_model.find("joint", "forearm_ty")
            forearm_tz = hand.mjcf_model.find("joint", "forearm_tz")
            
            if forearm_tx is not None:
                print(f"    forearm_tx: axis={forearm_tx.axis}, range={forearm_tx.range}")
            if forearm_ty is not None:
                print(f"    forearm_ty: axis={forearm_ty.axis}, range={forearm_ty.range}")
            if forearm_tz is not None:
                print(f"    forearm_tz: axis={forearm_tz.axis}, range={forearm_tz.range}")
    
    # Get piano dimensions
    if hasattr(env.task, 'piano'):
        piano = env.task.piano
        print(f"\nPiano Dimensions (size):")
        print(f"  X (depth):  {piano.size[0]:.3f} m")
        print(f"  Y (width):  {piano.size[1]:.3f} m (keyboard length)")
        print(f"  Z (height): {piano.size[2]:.3f} m")
    
    print(f"\nCoordinate System (MuJoCo convention):")
    print(f"  X-axis: Forward/backward (depth)")
    print(f"  Y-axis: Left/right (along keyboard length)")
    print(f"  Z-axis: Up/down (vertical)")
    print(f"\nForearm DOF Axes:")
    print(f"  forearm_tx: axis=(-1, 0, 0) - slides along X-axis (keyboard length)")
    print(f"  forearm_ty: axis=(0, 0, 1)  - slides along Z-axis (vertical)")
    print(f"  forearm_tz: axis=(0, 1, 0)  - slides along Y-axis (perpendicular to keyboard)")
    print("=" * 70 + "\n")
