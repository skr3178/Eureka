...
# Calculate EUREKA_ROOT_DIR first to identify local gymnasium path
EUREKA_ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
gymnasium_local_path = os.path.join(EUREKA_ROOT_DIR, 'gymnasium')

# Remove local gymnasium from sys.path BEFORE any imports to prevent conflicts
# This must happen before importing gymnasium
if gymnasium_local_path in sys.path:
    sys.path.remove(gymnasium_local_path)
# Also check if parent directory containing gymnasium is in path
if EUREKA_ROOT_DIR in sys.path:
    # Temporarily remove it, import gymnasium, then add it back
    sys.path.remove(EUREKA_ROOT_DIR)

import numpy as np
import torch
from omegaconf import OmegaConf

# Import gymnasium from system installation BEFORE adding local paths back
# This ensures we use the properly installed gymnasium, not the local editable one
import gymnasium as gym
from gymnasium.vector import SyncVectorEnv

# Now add EUREKA_ROOT_DIR back to path for other imports
# Ensure local gymnasium directory is never in path to avoid conflicts
    ...
    def after_init(self, algo):
        ...
        if hasattr(algo, 'writer') and algo.writer is not None:
            ...
            if hasattr(algo.writer, 'log_dir'):
                ...
                if self.output_dir:
                    log_dir = self.output_dir / "tensorboard" / algo.experiment_name / "summaries"
                else:
            ...
            if self.output_dir:
                # Use output directory for TensorBoard logs
                summaries_dir = self.output_dir / "tensorboard"
            else:
...
def load_generated_env(env_file_path):
    ...
    # It could be AntEnv, AntEnvGPT, RoboPianistEnvGPT, or any class ending with Env
    # But exclude the base gymnasium.Env class
    env_class = None
    env_candidates = []
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default=None,
                        help='Output directory for TensorBoard logs (default: runs/ in current directory)')
    ...
    # Try to detect environment name from class name, fallback to mujoco_ant
    ...
    if 'RoboPianist' in env_class_name or 'robopianist' in env_class_name.lower():
        ...
        # Default: use class name in lowercase with mujoco_ prefix
    ...
    # Create algo observer with output directory
    observer = MujocoAlgoObserver(output_dir=args.output_dir)
